...
def run_mv_review(mv_bytes, supporting_bytes, ref_no, client_name, esp_name, mv_filename, facility_name="", regression_data=None, regression_data_provided=False, debug_chunks=False):
    ...
    reported_by_eem = {}
    item_636 = review_by_sn.get("6.3.6", {})
    if isinstance(item_636.get("regression_stats"), dict):
        reported_by_eem = item_636["regression_stats"]
        logging.info("Extracted reported regression stats for %d EEM(s) from AI.", len(reported_by_eem))
...
def _parse_regression_excel(excel_bytes: bytes) -> list:
    ...
    _HEADER_WORDS = (
        ...
        "baseline", "driving", "cdd", "hdd", "#", "deg", "ambient",
        "temp", "humidity", "occupancy", "load", "chiller", "efficiency",
    )

    def _is_unit_string(val: str) -> bool:
        """Return True for strings that look like unit labels, e.g. '(Deg. C)', 'kW/TR'."""
        stripped = val.strip("() ")
        # Pure unit/symbol strings: short, contain slashes, dots, or degree symbols
        if len(val) <= 12 and any(ch in val for ch in ("/", "°", ".", "%")):
            return True
        # Wrapped entirely in parentheses
    ...
    for sheet_name in wb.sheetnames:
        ...
        while i < n_rows:
            ...
            # EEM name: scan upward for the first non-header, non-unit text cell
            eem_name = sheet_name
                ...
                for cell in rows[hi]:
                    if cell is not None:
                        ...
                        try:
                            ...
                            pass
                        if _is_unit_string(val):
                            continue   # skip unit labels like "(Deg. C)", "kW/TR"
            ...
            break   # one regression model per sheet — stop after first valid block
